...
try:
    ...
    # Create dummy modules to prevent startup failure
    class DummyModule:
        pass
    ai_insights = auth = crud = database = notifications = DummyModule()
    Subscription = SubscriptionCreate = SubscriptionUpdate = Token = User = UserCreate = UserInDB = NotificationPreferences = DummyModule
...
try:
    ...
    print("⚠️  The application will start but database operations will fail.")
    print("ℹ️  Please ensure MONGODB_URL is set correctly in environment variables.")
...
def _seed_demo_users() -> None:
    ...
    try:
        ...
        print(f"⚠️ Failed to seed demo users: {e}")
...
def _seed_demo_subscriptions() -> None:
    ...
    try:
        ...
        samples = [
            (SubscriptionCreate(service_name="Netflix", cost=15.99, billing_cycle="monthly", renewal_date=seven_days, notes="4K plan", is_shared=True), str(admin.id)),
            (SubscriptionCreate(service_name="Spotify", cost=9.99, billing_cycle="monthly", renewal_date=seven_days, notes="", is_shared=False), str(alice.id)),
            (SubscriptionCreate(service_name="Domain", cost=12.0, billing_cycle="yearly", renewal_date=seven_days, notes="example.com", is_shared=False), str(bob.id)),
        ]
        for sub, owner in samples:
            crud.create_subscription(sub, owner)
        print("✅ Demo subscriptions seeded successfully!")
    except Exception as e:
        print(f"⚠️ Failed to seed demo subscriptions: {e}")
...
@app.get("/")
async def root():
    return {"message": "Subscription Manager API", "status": "healthy"}
